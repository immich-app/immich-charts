...
*/ -}}
{{- define "server.controller.pod" -}}
  {{- with .Values.imagePullSecrets }}
imagePullSecrets:
    {{- toYaml . | nindent 2 }}
  {{- end }}
serviceAccountName: {{ include "server.names.serviceAccountName" . }}
...
initContainers:
  ...
  {{- end }}
containers:
  {{- include "server.controller.mainContainer" . | nindent 2 }}
  {{- with .Values.additionalContainers }}
    ...
    {{- range $name, $container := . }}
      ...
      {{- if $container.env -}}
        {{- $_ := set $ "ObjectValues" (dict "env" $container.env) -}}
        {{- $newEnv := fromYaml (include "server.controller.env_vars" $) -}}
        {{- $_ := set $container "env" $newEnv.env }}
        {{- $_ := unset $.ObjectValues "env" -}}
      {{- end }}
  ...
  {{- end }}
  {{- with (include "server.controller.volumes" . | trim) }}
volumes:
    {{- nindent 2 . }}
  {{- end }}
