...
*/}}
{{- define "machine_learning.service" -}}
  {{- /* Generate named services as required */ -}}
  {{- range $name, $service := .Values.machine_learning.service }}
    {{- if $service.enabled -}}
      {{- $serviceValues := $service -}}

      {{/* set the default nameOverride to the service name */}}
      {{- if and (not $serviceValues.nameOverride) (ne $name (include "machine_learning.service.primary" $)) -}}
        {{- $_ := set $serviceValues "nameOverride" $name -}}
      {{ end -}}

      {{- $_ := set $ "ObjectValues" (dict "service" $serviceValues) -}}
      {{- include "machine_learning.classes.service" $ }}
    {{- end }}
  {{- end }}
{{- end }}
...
*/}}
{{- define "machine_learning.service.primary" -}}
...
*/}}
{{- define "machine_learning.classes.service.ports.primary" -}}
  {{- $enabledPorts := dict -}}
...
*/}}
{{- define "machine_learning.servicename" -}}
{{- $values := .Values.machine_learning.service -}}
...
{{- $serviceName := include "machine_learning.names.fullname" . -}}
...
within the machine_learning.
*/}}
{{- define "machine_learning.classes.service" -}}
{{- $values := .Values.machine_learning.service -}}
{{- if hasKey . "ObjectValues" -}}
  {{- with .ObjectValues.service -}}
    {{- $values = . -}}
...
{{- $primaryPort := get $values.ports (include "machine_learning.classes.service.ports.primary" (dict "values" $values)) }}
---
apiVersion: v1
kind: Service
metadata:
  name: immich-machine-learning
  {{- with (merge ($values.labels | default dict) (include "machine_learning.labels" $ | fromYaml)) }}
  ...
  {{- end }}
  {{- with (merge ($values.annotations | default dict) (include "machine_learning.annotations" $ | fromYaml)) }}
    {{ toYaml . | nindent 4 }}
  {{- end }}
spec:
  ...
  {{- end }}
  selector:
    {{- include "machine_learning.labels.selectorLabels" . | nindent 4 }}
{{- end }}
