...
*/ -}}
{{- define "web.controller.pod" -}}
  {{- with .Values.imagePullSecrets }}
imagePullSecrets:
    {{- toYaml . | nindent 2 }}
  {{- end }}
serviceAccountName: {{ include "web.names.serviceAccountName" . }}
...
initContainers:
  ...
  {{- end }}
containers:
  {{- include "web.controller.mainContainer" . | nindent 2 }}
  {{- with .Values.additionalContainers }}
    ...
    {{- range $name, $container := . }}
      ...
      {{- if $container.env -}}
        {{- $_ := set $ "ObjectValues" (dict "env" $container.env) -}}
        {{- $newEnv := fromYaml (include "web.controller.env_vars" $) -}}
        {{- $_ := set $container "env" $newEnv.env }}
        {{- $_ := unset $.ObjectValues "env" -}}
      {{- end }}
