...
{{- define "microservice.service" -}}
  ...
  {{- range $name, $service := .Values.microservice.service }}
    {{- if $service.enabled -}}
      {{- $serviceValues := $service -}}

      {{/* set the default nameOverride to the service name */}}
      {{- if and (not $serviceValues.nameOverride) (ne $name (include "microservice.service.primary" $)) -}}
        {{- $_ := set $serviceValues "nameOverride" $name -}}
      {{ end -}}

      {{- $_ := set $ "ObjectValues" (dict "service" $serviceValues) -}}
      {{- include "microservice.classes.service" $ }}
    {{- end }}
  {{- end }}
{{- end }}
...
*/}}
{{- define "microservice.service.primary" -}}
...
*/}}
{{- define "microservice.classes.service.ports.primary" -}}
  {{- $enabledPorts := dict -}}
...
{{- $values := .Values.microservice.service -}}
...
{{- $serviceName := include "microservice.names.fullname" . -}}
...
*/}}
{{- define "microservice.classes.service" -}}
{{- $values := .Values.microservice.service -}}
{{- if hasKey . "ObjectValues" -}}
  {{- with .ObjectValues.service -}}
    {{- $values = . -}}
...
{{- $primaryPort := get $values.ports (include "microservice.classes.service.ports.primary" (dict "values" $values)) }}
---
apiVersion: v1
kind: Service
metadata:
{{/* Disabled until this is configurable inside the app name: {{ $serviceName }} */}} 
  name: immich-microservices
  {{- with (merge ($values.labels | default dict) (include "microservice.labels" $ | fromYaml)) }}
  ...
  {{- end }}
  {{- with (merge ($values.annotations | default dict) (include "microservice.annotations" $ | fromYaml)) }}
    {{ toYaml . | nindent 4 }}
  {{- end }}
spec:
  ...
  {{- end }}
  selector:
    {{- include "microservice.labels.selectorLabels" . | nindent 4 }}
{{- end }}
