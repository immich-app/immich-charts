...
{{- define "web.service" -}}
  ...
  {{- range $name, $service := .Values.web.service }}
    {{- if $service.enabled -}}
      {{- $serviceValues := $service -}}

      {{/* set the default nameOverride to the service name */}}
      {{- if and (not $serviceValues.nameOverride) (ne $name (include "web.service.primary" $)) -}}
        {{- $_ := set $serviceValues "nameOverride" $name -}}
      {{ end -}}

      {{- $_ := set $ "ObjectValues" (dict "service" $serviceValues) -}}
      {{- include "web.classes.service" $ }}
    {{- end }}
  {{- end }}
{{- end }}
...
*/}}
{{- define "web.classes.service.ports.primary" -}}
  {{- $enabledPorts := dict -}}
...
{{- $serviceName := include "web.names.fullname" . -}}
...
{{- $primaryPort := get $values.ports (include "web.classes.service.ports.primary" (dict "values" $values)) }}
---
apiVersion: v1
kind: Service
metadata:
  name: immich-web
  {{- with (merge ($values.labels | default dict) (include "web.labels" $ | fromYaml)) }}
  labels: {{- toYaml . | nindent 4 }}
  {{- end }}
  annotations:
  {{- if eq ( $primaryPort.protocol | default "" ) "HTTPS" }}
    traefik.ingress.kubernetes.io/service.websscheme: https
  {{- end }}
  {{- with (merge ($values.annotations | default dict) (include "web.annotations" $ | fromYaml)) }}
    {{ toYaml . | nindent 4 }}
  {{- end }}
spec:
  ...
  {{- end }}
  selector:
    {{- include "web.labels.selectorLabels" . | nindent 4 }}
{{- end }}
