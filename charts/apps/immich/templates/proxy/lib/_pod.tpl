...
*/ -}}
{{- define "proxy.controller.pod" -}}
  {{- with .Values.imagePullSecrets }}
imagePullSecrets:
    {{- toYaml . | nindent 2 }}
  {{- end }}
serviceAccountName: {{ include "proxy.names.serviceAccountName" . }}
...
initContainers:
  ...
  {{- end }}
containers:
  {{- include "proxy.controller.mainContainer" . | nindent 2 }}
  {{- with .Values.additionalContainers }}
    ...
    {{- range $name, $container := . }}
      ...
      {{- if $container.env -}}
        {{- $_ := set $ "ObjectValues" (dict "env" $container.env) -}}
        {{- $newEnv := fromYaml (include "proxy.controller.env_vars" $) -}}
        {{- $_ := set $container "env" $newEnv.env }}
        {{- $_ := unset $.ObjectValues "env" -}}
      {{- end }}
